...
@Component({
  ...
  template: `
    <div class="fixed bottom-6 right-6 z-[9999] flex flex-col items-end gap-4">
      ...
      <!-- Chat Window -->
      @if (isOpen()) {
        <div class="w-[380px] h-[520px] bg-white dark:bg-dark-elevated rounded-3xl shadow-2xl border border-slate-200 dark:border-white/10 flex flex-col overflow-hidden animate-chat-in backdrop-blur-xl card-premium">
          
          <!-- Header -->
          <div class="p-4 bg-gradient-to-r from-primary to-blue-600 text-white flex items-center justify-between shadow-lg">
            <div class="flex items-center gap-3">
              <div class="w-10 h-10 rounded-full bg-white/20 flex items-center justify-center backdrop-blur-md relative">
                <span class="text-lg font-black">{{ isCustomer() ? 'S' : 'C' }}</span>
                <div class="absolute bottom-0 right-0 w-2.5 h-2.5 bg-emerald-400 border-2 border-primary rounded-full"></div>
              </div>
              <div>
                <h3 class="text-sm font-black uppercase tracking-tight">{{ isCustomer() ? 'Support Team' : 'Customer Support' }}</h3>
                <p class="text-[10px] opacity-80 font-bold uppercase tracking-widest">Always Active</p>
              </div>
            </div>
            <button (click)="isOpen.set(false)" class="p-2 hover:bg-white/10 rounded-xl transition-all">
      ...
      >
        ...
        @if (unreadCount() > 0 && !isOpen()) {
          <div class="absolute -top-1 -right-1 w-6 h-6 bg-rose-500 text-white text-[10px] font-black rounded-full flex items-center justify-center border-2 border-white dark:border-dark-base animate-bounce">
            {{ unreadCount() }}
          </div>
        }
      </button>
