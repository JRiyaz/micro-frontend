import { Component, signal, inject, input, output, computed, ElementRef, viewChild, effect } from '@angular/core';
      ...
      >
        @if (chatService.allMessages().length === 0) {
          <div class="flex flex-col items-center justify-center h-full text-center p-8 animate-fade-in">
            ...
            <h3 [class]="isCompact() ? 'text-[11px]' : 'text-sm'" class="font-black text-slate-400 uppercase tracking-widest">No Messages Yet</h3>
          </div>
        } @else {
          @for (msg of chatService.allMessages(); track msg.id) {
            <div class="flex flex-col" [class.items-end]="msg.role === currentRole()">
              <div class="flex items-end gap-2" [class.flex-row-reverse]="msg.role === currentRole()">
                <div 
                  [class]="isCompact() ? 'w-6 h-6 text-[10px]' : 'w-8 h-8 text-xs'"
                  class="rounded-lg bg-slate-200 dark:bg-white/10 flex-shrink-0 flex items-center justify-center font-black shadow-sm"
                >
                  {{ msg.senderName.charAt(0) }}
                </div>
                <div 
                  [class]="msg.role === currentRole() ? 'bg-primary text-white rounded-xl rounded-tr-none shadow-md shadow-primary/10' : 'bg-white dark:bg-dark-card text-slate-800 dark:text-slate-200 rounded-xl rounded-tl-none border border-slate-200 dark:border-white/5 shadow-sm'"
        ...
        @if (showTyping()) {
          <div class="flex items-center gap-3 animate-fade-in pl-1">
            <div class="w-8 h-8 rounded-xl bg-slate-200 dark:bg-white/10 flex items-center justify-center">
            ...
            <span class="text-[9px] font-black text-slate-400 uppercase tracking-widest italic">{{ isCustomer() ? 'Employee' : 'Customer' }} is typing...</span>
          </div>
        }
      </div>

      <!-- Input Area -->
      <div class="p-6 bg-white dark:bg-dark-surface border-t border-slate-200 dark:border-white/5">
        <div class="relative flex items-center gap-4">
          ...
          >
            <svg class="w-6 h-6 rotate-45 -translate-y-0.5 -translate-x-0.5 group-hover:translate-x-0 group-hover:-translate-y-1 transition-transform" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2.5" d="M12 19l9 2-9-18-9 18 9-2zm0 0v-8"></path></svg>
          </button>
        </div>
      </div>
    </div>
