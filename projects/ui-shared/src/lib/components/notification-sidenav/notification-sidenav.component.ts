    ...
    <div class="fixed inset-0 z-[60] pointer-events-none">
      ...
      >
        ...
        <div class="p-6 border-b border-slate-200 dark:border-white/[0.06] flex items-center justify-between">
          <div>
            <h2 class="text-xl font-black tracking-tight text-slate-900 dark:text-white">Notifications</h2>
            <p class="text-xs text-slate-500 dark:text-slate-400 font-medium">Keep track of your latest updates</p>
          </div>
          <button (click)="close.emit()" class="p-2 text-slate-400 hover:text-slate-900 dark:hover:text-white transition-colors rounded-xl hover:bg-slate-100 dark:hover:bg-white/5">
        ...
        <div class="flex-1 overflow-y-auto p-4 space-y-3 sidebar-scroll">
          @for (n of notificationService.notifications(); track n.id) {
            <div 
              ...
              [ngClass]="{
                'bg-slate-50 dark:bg-white/[0.02] border-slate-200 dark:border-white/[0.06]': n.read,
                'bg-white dark:bg-white/[0.04] border-primary/20 dark:border-primary/30 shadow-sm': !n.read
              }"
            >
              <div class="flex gap-3">
                ...
                >
                  @if (n.type === 'success') { <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 13l4 4L19 7"></path></svg> }
                  @else if (n.type === 'error') { <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12"></path></svg> }
                  @else if (n.type === 'warning') { <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 9v2m0 4h.01m-6.938 4h13.856c1.54 0 2.502-1.667 1.732-3L13.732 4c-.77-1.333-2.694-1.333-3.464 0L3.34 16c-.77 1.333.192 3 1.732 3z"></path></svg> }
                ...
                <div class="flex-1 min-w-0 pr-6">
                  <h5 class="text-sm font-bold text-slate-900 dark:text-white leading-tight mb-1">{{ n.title }}</h5>
                  <p class="text-xs text-slate-500 dark:text-slate-400 leading-normal">{{ n.message }}</p>
                  <p class="text-[10px] text-slate-400 dark:text-slate-500 font-mono mt-2">{{ n.timestamp | date:'shortTime' }}</p>
                </div>
              </div>
            ...
            <div class="h-full flex flex-col items-center justify-center text-center p-8 opacity-50">
              ...
              <p class="text-sm font-bold text-slate-900 dark:text-white">All caught up!</p>
              <p class="text-xs text-slate-500 dark:text-slate-400 mt-1">No new notifications at the moment.</p>
            </div>
          }
        </div>

        <!-- Footer -->
        <div class="p-4 border-t border-slate-200 dark:border-white/[0.06] bg-slate-50/50 dark:bg-black/10">
          <button 
