...
@Component({
  ...
  template: `
    <span [ngClass]="statusClasses()" class="px-2.5 py-1 rounded-lg text-[9px] font-black uppercase tracking-widest border transition-all duration-300 flex items-center gap-1.5 w-fit">
      @if (isProcessing()) {
        <div class="dots-wave">
          <span></span>
          <span></span>
...
export class StatusBadgeComponent {
  ...
  statusClasses = computed(() => {
    ...
    if (['completed', 'active', 'shipped', 'delivered', 'received', 'optimal stock', 'in stock', 'low risk'].includes(s)) {
  ...
  isProcessing = computed(() => {
    const s = this.status().toLowerCase();
    return [
      'pending', 'processing', 'ordered', 'shipping', 'packaging', 
      'verifying', 'calculating', 'allocating', 'syncing', 'in progress'
    ].includes(s);
  });
}
