...
@Component({
  ...
  template: `
    <div class="floating-input-group w-full" id="datepicker-container">
      ...
      >
        ...
        <svg class="w-3.5 h-3.5 text-slate-400 group-hover:text-primary transition-transform duration-300" [class.rotate-180]="isOpen()" fill="none" stroke="currentColor" viewBox="0 0 24 24">
          <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 9l-7 7-7-7"></path>
        </svg>
      </button>

      <label class="floating-label" [class.floating-active]="formattedDate() || isOpen()">
        {{ label() || placeholder() }}
      </label>
      ...
      @if (isOpen()) {
        <div 
          ...
          <div class="absolute top-0 left-0 w-full h-1 bg-gradient-to-r from-primary to-blue-400 opacity-50"></div>

          <!-- Calendar Header -->
          <div class="flex items-center justify-between mb-6">
            <button (click)="prevMonth()" class="p-2 hover:bg-primary/10 rounded-xl text-slate-400 hover:text-primary transition-all active:scale-90">
              <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 19l-7-7 7-7"></path></svg>
            </button>
            <div class="flex flex-col items-center">
              <span class="text-[10px] font-black uppercase tracking-[0.2em] text-primary mb-0.5">
                {{ viewDate().getFullYear() }}
              </span>
              <span class="text-sm font-black uppercase tracking-tight text-slate-900 dark:text-white">
                {{ monthNames[viewDate().getMonth()] }}
              </span>
            </div>
            <button (click)="nextMonth()" class="p-2 hover:bg-primary/10 rounded-xl text-slate-400 hover:text-primary transition-all active:scale-90">
          ...
          <div class="grid grid-cols-7 mb-3">
            @for (day of weekDays; track day) {
              <div class="text-center text-[9px] font-black text-slate-400 dark:text-slate-500 uppercase tracking-widest py-1">
                {{ day }}
              </div>
            }
          </div>
          ...
          <!-- Today shortcut -->
          <div class="mt-6 pt-4 border-t border-slate-100 dark:border-white/[0.05]">
            <button (click)="selectToday()" class="w-full py-2 text-[10px] font-black uppercase tracking-widest text-slate-500 hover:text-primary transition-colors text-center">
              Go to Today
            </button>
          </div>
        </div>
  ...
  styles: [`
    ...
    .floating-input {
      ...
      border-bottom: 2px solid color-mix(in srgb, currentColor, transparent 90%) !important;
    ...
    .calendar-selected {
      ...
      box-shadow: 0 4px 12px color-mix(in srgb, var(--theme-primary), transparent 60%);
    }
