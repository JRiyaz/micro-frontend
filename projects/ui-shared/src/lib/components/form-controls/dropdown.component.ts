    ...
    <div class="relative w-full" id="dropdown-container">
      ...
      >
        <span class="flex items-center gap-3">
          <div *ngIf="selectedOption()?.color" [style.background]="selectedOption()?.color" class="w-2 h-2 rounded-full"></div>
      ...
      <div *ngIf="isOpen()" 
        ...
        <div class="max-h-60 overflow-y-auto custom-scrollbar">
          ...
          >
            <div *ngIf="option.color" [style.background]="option.color" class="w-2 h-2 rounded-full"></div>
            <span [innerHTML]="option.icon" *ngIf="option.icon" class="w-4 h-4 text-slate-400"></span>
            {{ option.label }}
          </button>
        </div>
      </div>
