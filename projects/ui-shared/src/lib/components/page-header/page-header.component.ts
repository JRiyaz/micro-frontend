    ...
    <div class="mb-6 animate-fade-in">
        ...
        <nav class="flex items-center gap-2 mb-3 px-1">
          @for (item of breadcrumbs(); track item.label; let last = $last) {
            <div class="flex items-center gap-2">
              @if (item.link) {
                <a [routerLink]="item.link" class="text-[9px] font-black uppercase tracking-widest text-slate-400 hover:text-primary transition-colors">{{ item.label }}</a>
              } @else {
                <span class="text-[9px] font-black uppercase tracking-widest text-slate-500">{{ item.label }}</span>
              }
              @if (!last) {
                <svg class="w-2.5 h-2.5 text-slate-300" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="3" d="M9 5l7 7-7 7"/></svg>
              }
            </div>
          }
        </nav>
      ...
      <div class="flex flex-col lg:flex-row lg:items-center justify-between gap-4 mb-6">
        ...
        <div class="flex items-center gap-3">
          <!-- Total Count Display -->
          @if (count() !== null) {
            <div class="flex items-center gap-2 px-4 py-2 bg-slate-50 dark:bg-white/5 border border-slate-200 dark:border-white/10 rounded-lg shadow-inner group transition-all hover:border-primary/30">
              ...
              <div class="min-w-[20px] flex justify-center">
                @if (loading()) {
                  ...
                  <span class="text-xs font-black text-slate-900 dark:text-white animate-scale-in">{{ count() }}</span>
                }
              </div>
            </div>
          }
        ...
        @for (stat of stats(); track stat.label) {
          <div 
            ...
            [ngClass]="{
              ...
              'border-l-primary': !loading() && (stat.color === 'primary' || !stat.color)
            }"
          >
             <div>
                ...
                <div class="h-6 flex items-center">
                  @if (loading()) {
                    ...
                    <p class="text-lg font-black text-slate-900 dark:text-white leading-none animate-scale-in">{{ stat.value }}</p>
                  }
                </div>
             </div>
             <div 
               ...
               [ngClass]="loading() ? 'bg-slate-50 dark:bg-white/5 text-transparent' : {
                 'bg-emerald-500/5 text-emerald-500': stat.color === 'success',
                 ...
                 'bg-primary/5 text-primary': stat.color === 'primary' || !stat.color
               }"
             >
                @if (!loading()) {
                  @if (stat.icon) {
                    <div [innerHTML]="sanitize(stat.icon)" class="w-5 h-5 animate-scale-in flex items-center justify-center"></div>
                  } @else {
                    @if (stat.color === 'danger') {
                      <svg class="w-5 h-5 animate-scale-in" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M13 17h8m0 0V9m0 8l-8-8-4 4-6-6"/></svg>
                    } @else {
                      <svg class="w-5 h-5 animate-scale-in" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M13 7h8m0 0v8m0-8l-8 8-4-4-6 6"/></svg>
                    }
                  }
                }
             </div>
